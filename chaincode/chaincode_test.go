...
func getByteReqest(t *testing.T) []byte {
	req := define.Factory{Key: "key", Value: "v"}
	bytes, err := req.MarshalJSON()
	if err != nil {
		t.Fatal("factory marshal failed")
		return nil
	}
	return bytes
}
...
func checkQuery(t *testing.T, stub *shimtest.MockStub, name string, value string) {
	res := stub.MockInvoke("1", [][]byte{[]byte("QueryData"), []byte(name)})
	if res.Status != shim.OK {
...
func TestFactorycc_Init(t *testing.T) {
	cc := new(FactoryChaincode)
	stub := shimtest.NewMockStub("mycc", cc)

	// Init a=10
	checkInit(t, stub, nil)

	checkState(t, stub, "keepalive", "keepalive")
}

func TestFactorycc_Query(t *testing.T) {
	cc := new(FactoryChaincode)
	stub := shimtest.NewMockStub("mycc", cc)

	// Init a=10
	checkInit(t, stub, [][]byte{[]byte("keepalive"), []byte("keepalive")})

	// Query a
	checkQuery(t, stub, "keepalive", "keepalive")
}

func TestFactorycc_Invoke(t *testing.T) {
	cc := new(FactoryChaincode)
	stub := shimtest.NewMockStub("mycc", cc)
	...
	// Invoke: Set a=20
	checkInvoke(t, stub, [][]byte{[]byte("SaveData"), getByteReqest(t)})

	// Query a
	checkQuery(t, stub, "key", string(getByteReqest(t)))
}

//func TestFactorycc_InitWithIncorrectArguments(t *testing.T) {
//	cc := new(FactoryChaincode)
//	stub := shimtest.NewMockStub("mycc", cc)
//
...
//}
//
//func TestFactorycc_QueryWithIncorrectArguments(t *testing.T) {
//	cc := new(FactoryChaincode)
//	stub := shimtest.NewMockStub("mycc", cc)
//
//	// Init a=10
...
//}
//
//func TestFactorycc_QueryForAssetNotFound(t *testing.T) {
//	cc := new(FactoryChaincode)
//	stub := shimtest.NewMockStub("mycc", cc)
//
//	// Init a=10
...
//}
//
//func TestFactorycc_InvokeWithIncorrectArguments(t *testing.T) {
//	cc := new(FactoryChaincode)
//	stub := shimtest.NewMockStub("mycc", cc)
//
//	// Init a=10
